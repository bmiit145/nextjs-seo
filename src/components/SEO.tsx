...
export const SEO: React.FC<SEOProps> = ({
  ...
  useEffect(() => {
    ...
    if (openGraph) {
      if (openGraph.title) updateMetaTag("og:title", openGraph.title);
      if (openGraph.description) updateMetaTag("og:description", openGraph.description);
      if (openGraph.url) updateMetaTag("og:url", openGraph.url);
      if (openGraph.type) updateMetaTag("og:type", openGraph.type);
    ...
    if (twitter) {
      ...
      if (twitter.handle) updateMetaTag("twitter:creator", twitter.handle);
    }
    ...
    if (robotsProps) {
      ...
      for (const [key, value] of Object.entries(robotsProps)) {
        if (value) robotsParams += `,${key}:${value}`;
      }
